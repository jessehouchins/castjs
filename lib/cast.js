  ...
  cast.register({
    ...
    date: {
      ...
      before: function(x, limit){
        if (Date.parse(x) >= Date.parse(limit)) this.set(null)
      },
...
cast(attrs, 'start_date').date().beginningOfDay().before(attrs.end_date).format('yyyy-mm-dd HH:MM:SS').when({
  blank: function(){ console.log('Start Date is required.') },
  invalid: function(x){ console.log('Start Date must be before End Date') },
  valid: function(x){ console.log('Start Date ('+x+') is valid.') }
})

cast(attrs, 'end_date').date().endOfDay().after(attrs.start_date).format('yyyy-mm-dd HH:MM:SS').when({
  blank: function(){ console.log('End Date is required.') },
  invalid: function(x){ console.log('End Date must be after Start Date') },
  valid: function(x){ console.log('End date ('+x+') is valid.') }
})
